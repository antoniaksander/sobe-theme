...
export default function Edit({ attributes, setAttributes }) {
  ...
  return (
    <>
      <InspectorControls>
        <PanelBody title={__('Display', 'sobe')} initialOpen={true}>
          <PanelRow>
            <ToggleControl
              label={__('Show brand logos', 'sobe')}
              help={showLogos
                ? __('Logos shown where available.', 'sobe')
                : __('Brand names only.', 'sobe')}
              checked={showLogos}
          ...
          <PanelRow>
            <ToggleControl
              label={__('Hide brands with no products', 'sobe')}
              checked={hideEmpty}
